"""Entry point: one shape, one sweep, one batch.

Stub created by P0.T18 (`spec-harness.md` §1). Implementation lands in P0.T19.
Module rooting is fixed: this module is always addressed as
`fuzz.gw5ast138c.harness.__main__` and run from `$FL/apicula`; it never depends
...
def build_parser():
    ...
    """
    parser = argparse.ArgumentParser(prog="fuzz.gw5ast138c.harness.__main__")
    parser.add_argument(
        "--design-dir",
        required=True,
...
def main(argv=None):
    """Stub entry point. Implemented by P0.T19."""
    raise NotImplementedError(
        "fuzz.gw5ast138c.harness.__main__.main is a P0.T18 stub; implemented by P0.T19"
    )
